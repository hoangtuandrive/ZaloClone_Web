...
function MessageListSelected() {
  ...
  const { selectedRoomId, setIsModalOpenGroup , isEmojiPickerOpen,RenderContent } = useContext(AppContext);
  ...
  const deleteMess= async (mess) =>{
    ...
    if(currentUser.attributes.sub === mess.userID){
      await DataStore.delete(MessageModel,(messmodel)=> messmodel.id("eq",mess.id));
      setRender(!render);
    }
    else{
